...
class AEVTNet(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.classifier = nn.Sequential(nn.Linear(adjust_channels,simsiam_dim, bias=False),
    ...
    def connector(self, template_features: torch.Tensor, self_attention_features: torch.Tensor, cross_attention_features: torch.Tensor, gaussian_val: None or torch.Tensor) -> Dict[str, torch.Tensor]:
        bbox_pred, cls_pred, _, _ = self.connect_model(self_attention_features, cross_attention_features, template_features, gaussian_val)
