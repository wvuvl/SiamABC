...
class BBoxCropWithOffsets(A.DualTransform):
    ...
    def __init__(
        ...
        self.scale_x = random.uniform(min(self.scale), max(self.scale))
        self.scale_y = random.uniform(min(self.scale), max(self.scale))
        self.shift_x = random.uniform(min(self.shift), max(self.shift))
        self.shift_y = random.uniform(min(self.shift), max(self.shift))
    ...
    def get_params_dependent_on_targets(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        new_x = max(0, x - self.scale_x * w / 2 + self.shift_x)
        new_y = max(0, y - self.scale_y * h / 2 + self.shift_y)
        new_w = min(img_w, new_x + w + self.scale_x * w) - new_x
        new_h = min(img_h, new_y + h + self.scale_y * h) - new_y
