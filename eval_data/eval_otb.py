...
def eval_auc(json_path, dataset='OTB2015', result_path='./test/', tracker_reg='S*', start=0, end=1e6):
    list_path =json_path # os.path.join(realpath(dirname(__file__)), '../../', 'dataset', dataset + '.json')
    ...
    for i in range(n_seq):
        ...
        for j in range(len(trackers)):
            ...
            success_overlap[i][j] = compute_success_overlap(gt_rect[:min_num], bb[:min_num])
...
def eval_auc_tune(result_path, json_path):
    list_path =json_path
    annos = json.load(open(list_path, 'r'))
    seqs = list(annos.keys())  # dict to list for py3
    n_seq = len(seqs)
...
if __name__ == "__main__":
    ...
    print(eval_auc_tune( result_path,json_path))
