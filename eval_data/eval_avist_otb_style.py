...
class AVIST(object):
    def __init__(self, root_dir, json_file):
        super(AVIST, self).__init__()
        self.annos = json.load(open(json_file, 'r'))
        self.seq_names = list(self.annos.keys())  # dict to list for py3
        self.root_dir = root_dir
    
    def __getitem__(self, index):
        img_files = [os.path.join(self.root_dir, im_f) for im_f in self.annos[self.seq_names[index]]['img_names']]
...
class ExperimentAVIST(ExperimentOTB):

    def __init__(self, root_dir, json_file, version='AVIST',
                 result_dir='results', report_dir='reports'):
        
        self.dataset = AVIST(root_dir, json_file)
